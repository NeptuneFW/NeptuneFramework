...
{
  protected static \$table_id_column = 'id';
  private \$where = [];
  ...
  {";
    ...
    ';
      ...
      {
        $tableFileContent .= '
    $column' . ucfirst($column['Field']) . '->tdefault(\''. $column['Default'] .'\');
    ';
      }
  ...
  {
    \$this->where[\"first\"] = [\$column, \$operator, \$value];
    return \$this;
  }
  public function orWhere(\$column, \$operator, \$value)
  {
    \$this->where[\"orWhere\"][]= [\$column, \$operator, \$value];
    return \$this;
  }
  public function andWhere(\$column, \$operator, \$value)
  {
    \$this->where[\"andWhere\"][]= [\$column, \$operator, \$value];
    return \$this;
  }
  ...
  public static function orderBy($type, $limit = null, $where = null)
  {
  ...
  public static function find($id, $columns = null)
  {
    ...
    $i = 0;
    $'.$table[0] . 'Row->triton_' . $table[0] . '_id = [$id, $tableID];
...
class ". ucfirst($table[0]) . "Row
{
  ...
  public function first()
  {
    ...
    foreach($funcArgs as $funcArgKey => $funcArgValue)
    {
      ...
      {
          $rowFileContent .= "
      \$rowClass->". $funcArgValue ." =  \$this->" . $funcArgValue . "[0];
      ";
      }
    }
  ...
  public function save()
  {
    ...
    foreach($funcArgs as $funcArgKey => $funcArgValue)
    {
      ...
      {
        $rowFileContent .= "
        \$args['". $funcArgValue ."'] =  \$this->" . $funcArgValue . ';';
      }
    }
    $rowFileContent .= "
        foreach(\$args as \$funcArgKey => \$funcArgValue)
  ...
  public function delete()
  {
    ...
    if(file_exists("database/databases/" . $database . "/" . ucfirst($table[0]) . 'Table.php'))
    {
      ...
      {
        echo 'database/databases/' . $database . '/' . ucfirst($table[0]) . 'Table.php created successfuly.';
      }
    }
    if(file_exists("database/databases/" . $database . "/" . ucfirst($table[0]) . 'Row.php')) {
      if(file_get_contents("database/databases/" . $database . "/" . ucfirst($table[0]) . 'Row.php') == $rowFileContent) {
        echo 'database/databases/' . $database . '/' . ucfirst($table[0]) . 'Row.php created successfuly.';
      }
    }
  }
